...
<div align="center" class="pt-3">
    ...
    <div id="table" class="p-2">
        <table class="m-3" width="93%" style="font-size: 13px;">
            ...
            @if (count($invoicedetails) > 0)
                @foreach ($invoicedetails as $invoicedetail)
                    <tr data-entry-id="{{ $invoicedetail->id }}">
                        ...
                        <td field-key='unit_price' align="right" style="padding: 3px; padding-right: 5px">{{number_format(($invoicedetail->unit_price * $invoicedetail->exchange->riels),2, ',', ' ') }}</td>
                        <td field-key='total' align="right" style="padding: 3px; padding-right: 5px">{{number_format(($invoicedetail->total * $invoicedetail->exchange->riels),2, ',', ' ') }}</td>

                       <?php $riels=$invoicedetail->exchange->riels; ?>
            ...
            <tfoot align="right" >
                <tr>
                    <td colspan="5" align="right" style="padding-right: 5px"><b>សុរប រៀល(R):</b></td>
                    <td style="padding-right: 5px"><b>{{number_format(($invoice->invoicedetail->sum('total') * $riels),2, ',', ' ')}}</b></td>
                </tr>
            </tfoot>
        </table>
    </div>
