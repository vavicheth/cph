...
@section('content')
    <h3 class="page-title">Reports by Patient Type</h3>
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            ...
            <table class="table table-bordered table-striped {{ count($invoices) > 0 ? 'datatable' : '' }} @can('patient_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                ...
                <tr>
                    <th rowspan="2" ></th>
                    <th rowspan="2" style="text-align: center !important;">ថ្ងៃទី</th>
                ...
                @if (count($invoices) > 0)
                    @php
                        ...
                        for ($i=$from;$i<=$to;date_add($i, date_interval_create_from_date_string('1 days'))) {
                            $day=date_format($i, 'd');
                            echo "<tr data-entry-id='".$day."'>";
                            echo "<td></td>";
                            echo "<td field-key='date' style='text-align: center !important;'>".$day."</td>";

                            /* BS2 OPD */
                            $bs2_opd_count=App\Invoice::where('date',$i)->where('invstate_id','1')->count();
                            echo "<td field-key='bs2_opd_count'>".$bs2_opd_count."</td>";
                             $bs2_opd_total=App\Invoice::where('date',$i)->where('invstate_id','1')->sum('total');
                            echo "<td field-key='bs2_opd_total'>".number_format($bs2_opd_total,2)."</td>";

                            /* BS2 accident */
                            $bs2_accident_count=App\Invoice::where('date',$i)->where('invstate_id','4')->count();
                            echo "<td field-key='bs2_accident_count'>".$bs2_accident_count."</td>";
                             $bs2_accident_total=App\Invoice::where('date',$i)->where('invstate_id','4')->sum('total');
                            echo "<td field-key='bs2_ipd_total'>".number_format($bs2_accident_total,2)."</td>";



                            /* HEF OPD */
                            $hef_opd_count=App\Invoice::where('date',$i)->where('invstate_id','3')->count();
                            echo "<td field-key='hef_opd_count'>".$hef_opd_count."</td>";
                             $hef_opd_total=App\Invoice::where('date',$i)->where('invstate_id','3')->sum('total');
                            echo "<td field-key='hef_opd_total'>".number_format($hef_opd_total,2)."</td>";

                            /* HEF IPD */
                            $hef_ipd_count=App\Invoice::where('date',$i)->where('invstate_id','2')->count();
                            echo "<td field-key='hef_ipd_count'>".$hef_ipd_count."</td>";
                             $hef_ipd_total=App\Invoice::where('date',$i)->where('invstate_id','2')->sum('total');
                            echo "<td field-key='hef_ipd_total'>".number_format($hef_ipd_total,2)."</td>";


                            /* Total Original Price */
                            $invs=App\Invoice::where('date',$i)->whereIn('invstate_id',['1','2','3','4'])->get();
                            $total_org_price=0;
                            foreach ($invs as $inv){
                            ...
                            /* Count Patient */
                            $patient_count=App\Invoice::where('date',$i)->whereIn('invstate_id',['1','2','3','4'])->count();
                            echo "<td field-key='forte_count'>".$patient_count."</td>";

                            /* Total Price */
                            $total_price=App\Invoice::where('date',$i)->whereIn('invstate_id',['1','2','3','4'])->sum('total');
